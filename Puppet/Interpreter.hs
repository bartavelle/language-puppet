...
-- result of the computation (either an error, or a tuple containing all
...
getCatalog :: Monad m
           ...
           -> InterpreterReader m -- ^ The whole environment that is required in order to compute a catalog
           -> Nodename
           -> Facts
           -> m (Pair (S.Either PrettyError (FinalCatalog, EdgeMap, FinalCatalog, [Resource]))  [Pair Priority Doc])
getCatalog convertMonad interpretReader node facts   = do
    (output, _, warnings) <- convertMonad interpretReader (initialState facts) (computeCatalog node)
