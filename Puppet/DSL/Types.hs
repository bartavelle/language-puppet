...
convertTopLevel :: Statement -> Either Statement (TopLevelType, T.Text, Statement)
...
data Value
    ...
    -- the manifest.
    | VariableReference !T.Text
    | Empty
    | ResourceReference !T.Text !Expression -- restype resname
...
data Statement
    = Node T.Text ![Statement] !SourcePos -- ^ This holds the node name and list of statements.
    -- | This holds the variable name and the expression that it represents.
    | VariableAssignment !T.Text !Expression !SourcePos
    | Include !Expression !SourcePos
    | Import !T.Text !SourcePos
    | Require !T.Text !SourcePos
    -- | This holds the resource type, name, parameter list and virtuality.
    | Resource !T.Text !Expression ![(Expression, Expression)] !Virtuality !SourcePos
    ...
    -}
    | ResourceDefault !T.Text ![(Expression, Expression)] !SourcePos
    {-| This works like 'Resource', but the 'Expression' holds the resource
    name.
    -}
    | ResourceOverride !T.Text !Expression ![(Expression, Expression)] !SourcePos
    ...
    -}
    | ClassDeclaration !T.Text !(Maybe T.Text) ![(T.Text, Maybe Expression)] ![Statement] !SourcePos
    {-| The define declaration is like the 'ClassDeclaration' except it can't
    inherit from anything.
    -}
    | DefineDeclaration !T.Text ![(T.Text, Maybe Expression)] ![Statement] !SourcePos
    ...
    -}
    | ResourceCollection !T.Text !Expression ![(Expression, Expression)] !SourcePos
    -- |Same as 'ResourceCollection', but for \<\| \|\>.
    | VirtualResourceCollection !T.Text !Expression ![(Expression, Expression)] !SourcePos
    | DependenceChain !(T.Text,Expression) !(T.Text,Expression) !SourcePos
    | MainFunctionCall !T.Text ![Expression] !SourcePos
    ...
    -}
    | TopContainer ![(T.Text, Statement)] !Statement -- magic statement that is used to embody the spurious top level statements
    deriving(Show, Ord, Eq)
...
data Expression
	...
	| ResolvedResourceReference !T.Text !T.Text -- ^ Resolved resource reference
...
capitalizeResType = T.intercalate "::" . map capitalize' . T.splitOn "::"

capitalize' :: T.Text -> T.Text
capitalize' "" = ""
capitalize' t = T.cons (toUpper (T.head t)) (T.tail t)
