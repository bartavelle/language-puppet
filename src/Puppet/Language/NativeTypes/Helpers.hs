...
checkParameterList validparameters res
  | HS.null validparameters = Right res
  | otherwise =
      if HS.null setdiff
        then Right res
...
inrange mi ma param res =
  let va = res ^. rattributes . at param
   ...
   in case (va, na) of
        (Nothing, _) -> Right res
        (_, Just v) ->
          if (v >= fromIntegral mi) && (v <= fromIntegral ma)
            then Right res
...
validateSourceOrContent res =
  let parammap = res ^. rattributes
