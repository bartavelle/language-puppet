...
instance Pretty UnresolvedValue where
    ...
    pretty (UInterpolable v) = pretty '"' <> hcat (map specific (V.toList v)) <> pretty '"'
        where
            ...
            specific (Lookup (Terminal (UVariableReference vr)) (Terminal x)) = dullblue (ppline ("${" <> vr <> "[") <> pretty x <> "]}")
...
instance Pretty HOLambdaCall where
    pretty (HOLambdaCall hf me bp stts mee) = pretty hf <> mme <+> pretty bp <+> nest 2 (pretty '{' <> line <> ppStatements stts <> mmee) <$> pretty '}'
        where
            mme = case me of
