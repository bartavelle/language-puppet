...
finalize rx = do
  ...
  let getOver = use (scopes . ix scp . scopeOverrides) -- retrieves current overrides
      addResDefaults r = ifoldlM (addAttribute CantReplace) r resdefval
        where
          resdefval = resdefaults ^. ix (r ^. rid . itype) . resDefValues
...
loadParameters ::
  Container PValue -> -- Resource attributes (resolved)
    ...
    Just pp -> case incltype of
      ...
      _ ->
        throwPosError $
          "Can't include class"
            <+> ppline name'
            <+> "twice when using the resource-like syntax (first occurence at"
            <+> showPPos pp <> ")"
    Nothing -> do
