...
import Text.Megaparsec (Parsec, parse, satisfy, sepBy, sepBy1)
...
data HieraQueryType
  ...
  | -- | Combines array and scalar values to return a merged, flattened array with all duplicate removed.
    QUnique
  | -- | Combines the keys and values of any number of hashes to return a merged hash.
...
varSplitter ::
  Text ->
  -- | returns variable parts, function arguments
  HieraVar
varSplitter v =
...
instance FromJSON HieraConfigFile where
  parseJSON =
    let mkHiera5 :: Object -> Yaml.Parser HieraConfigFile
        mkHiera5 v = do
     ...
     in Aeson.withObject "v3 or v5" $ \o ->
          o .:? "version" >>= \case
...
query HieraConfigFile {_version, _backends, _hierarchy} fp cache vars hquery qt = do
  ...
  mvals <- liftIO $ forM searchin $ \((decodefunction, datadir, extension), h) -> do
    let extension' =
          if snd (FilePath.splitExtension h) == ".yaml"
            then ""
            else extension
